...
public class MoonPhaseOverlay implements IIngameOverlay {
    ...
    @Override
    public void render(ForgeIngameGui gui, PoseStack poseStack, float partialTicks, int w, int h) {
        ...
        if (client != null && level != null && config.indicatorVisibility) {
            if (ShowIndicatorUtils.showIndicatorInCurrentDimension(client)) {
                ...
                GuiComponent.drawString(poseStack, textRenderer, new TranslatableComponent(MOON_PHASE_KEY_PREFIX + phase), 0, 0, 0xffffff);
    ...
    public static void register() {
        IIngameOverlay overlay = ForgeIngameGui.HOTBAR_ELEMENT;
        if (overlay != null) {
            OverlayRegistry.registerOverlayAbove(overlay, MoonPhaseIndicator.MOD_ID, new MoonPhaseOverlay());
        }
    }
}
